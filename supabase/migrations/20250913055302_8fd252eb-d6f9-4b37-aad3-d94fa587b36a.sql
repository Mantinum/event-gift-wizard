-- 1) Ajout des colonnes dérivées pour l'âge
ALTER TABLE persons
  ADD COLUMN age_years int,
  ADD COLUMN age_months int,
  ADD COLUMN age_bucket text,
  ADD COLUMN is_minor boolean,
  ADD COLUMN next_birthday date,
  ADD COLUMN age_updated_at timestamptz;

-- 2) Fonction utilitaire pour calculer tous les champs d'âge
...
-- 3) Trigger pour calculer automatiquement les champs d'âge
...
-- Suppression du trigger s'il existe et création du nouveau
DROP TRIGGER IF EXISTS trg_persons_age_before_write ON persons;
CREATE TRIGGER trg_persons_age_before_write
BEFORE INSERT OR UPDATE OF birthday ON persons
FOR EACH ROW EXECUTE FUNCTION public.persons_age_before_write();

-- 4) Fonction pour la mise à jour quotidienne des anniversaires
...
-- 5) Mise à jour de toutes les personnes existantes avec les nouveaux champs
UPDATE persons 
